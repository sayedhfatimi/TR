<?php
	...
	mysql_select_db($mysql_database, $connect);
		if(isset($_COOKIE['ID_my_site'])) {
			...
			session_start();
			if (!isset($_SESSION['random_key']) || strlen($_SESSION['random_key'])==0){ $_SESSION['random_key'] = strtotime(date('Y-m-d H:i:s')); $_SESSION['user_file_ext']= ""; }
			...
			if (file_exists($large_image_location)){
				if(file_exists($thumb_image_location)){ $thumb_photo_exists = "<img src=\"".$upload_path.$thumb_image_name.$_SESSION['user_file_ext']."\" alt=\"Thumbnail Image\"/>"; }else{ $thumb_photo_exists = ""; }
				$large_photo_exists = "<img src=\"".$upload_path.$large_image_name.$_SESSION['user_file_ext']."\" alt=\"Large Image\"/>";
					} else { $large_photo_exists = ""; $thumb_photo_exists = ""; }
			...
			if (isset($_POST["upload_thumbnail"]) && strlen($large_photo_exists)>0) {
				...
				$update = mysql_query("UPDATE members SET pimage='$thumb_image_location' WHERE username='".$_COOKIE['ID_my_site']."'");
				header("location:user.php?user=".$_COOKIE['ID_my_site']."");
				exit(); }
...
<?php mysql_close($connect); } else { mysql_close($connect); header("Location: login.php"); } ?>